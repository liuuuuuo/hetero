...
if __name__=="__main__":
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        pred.load_state_dict(checkpoint['pred_state_dict'])
    ...
    for e in range(start_epoch, args.num_comms): # 从 start_epoch 开始
        model.train()
        pred.train()
        loss = None
        ...
        tb_writer.add_scalar('Train/Loss',loss.item() / len(train_gs), e)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        print('In epoch {}, loss: {}'.format(e, loss.item() / len(train_gs)))
        ...
        if (e + 1) % 5 == 0:
            ...
            # 定期保存断点文件
            ckpt_path = os.path.join(args.log_dir, f'checkpoint_epoch_{e}.pth')
            torch.save({
                'epoch': e,
